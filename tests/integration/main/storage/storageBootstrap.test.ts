...
import { LEGACY_EVENTS_FILE_NAME, LEGACY_EVENTS_MIGRATED_SUFFIX, SETTINGS_FILE_NAME } from '../../../../src/main/constants';
...
describe('openStorage across restarts', () => {
  it('imports legacy events once, normalizes them and keeps them after a restart', () => {
    ...
    expect(imported[0]).toMatchObject({ id: 'legacy-1', title: 'Zahnarzt', color: 'red', reminderMinutes: 15, notified: false });
  ...
  it('keeps events saved in one session visible in the next', () => {
    ...
    first.events.markNotified(saved.id);
    shutdown(first);
    ...
    expect(second.events.getPendingReminders().map((event) => event.id)).toEqual(['legacy-1']);
  });
});
